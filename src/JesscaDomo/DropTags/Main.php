...
class Main extends PluginBase implements Listener
{
	...
	public function onDataPacketSend(DataPacketSendEvent $event): void
	{
		...
		if ($packet instanceof BatchPacket) {
			...
			foreach ($packet->getPackets() as $buf) {
				$pk = PacketPool::getPacketById(ord($buf{
					0}));
				if ($pk instanceof MoveActorAbsolutePacket) {
					...
					if ($entity instanceof ItemEntity) {
						foreach ($level->getNearbyEntities($entity->getBoundingBox()->expandedCopy(2, 2, 2), $entity) as $nearbyEntity) {
	...
	public function onItemSpawn(ItemSpawnEvent $event): void
	{
		...
		$entity->setNameTag("" . $entity->getItem()->getCount() . "§ex§6 ");
